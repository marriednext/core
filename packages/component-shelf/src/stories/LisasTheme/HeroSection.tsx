...
export function HeroSection({ nameA, nameB, eventDate, location, imageUrl, imageComponent }: HeroSectionProps) {
    ...
    <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden">
      <div className="absolute inset-0">
        {imageComponent ? (
          ...
          <img src={imageUrl || "/romantic-vineyard-wedding-venue-at-golden-hour-wit.jpg"} alt="" className="w-full h-full object-cover" />
        )}
      ...
      >
        ...
        <h1 className="font-serif text-white">
          {nameA && <span className="block text-7xl md:text-8xl lg:text-9xl font-light italic mb-2">{nameA}</span>}
          <span className="block text-2xl md:text-3xl tracking-[0.5em] uppercase font-sans font-light my-6">&</span>
          {nameB && <span className="block text-7xl md:text-8xl lg:text-9xl font-light italic">{nameB}</span>}
        </h1>

        {formattedDate && (
          <div className="mt-12 flex items-center justify-center gap-8 text-white/90">
            <span className="w-16 h-px bg-white/40" />
            <p className="tracking-[0.3em] uppercase text-sm">{formattedDate}</p>
            <span className="w-16 h-px bg-white/40" />
          </div>
        )}

        {location && <p className="mt-4 text-white/80 tracking-[0.2em] uppercase text-sm">{location}</p>}
      </div>
